...
For `ESP-IDF-Web Monitor` command, the baud rate used is determined from build directory's `project_description.json` field called `monitor_baud` when that file exists. Otherwise the `idfWeb.monitorBaudRate` setting is used.
